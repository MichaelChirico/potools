...
Instead it's better to generate the complete message in a single string using `glue()` or `sprintf()` to interpolate in the parts that vary:
...
While Mongolian uses the same times of day, it arranges the words in the opposite order to English: "Ogloonii mend" is morning greetings, "Odriin mend" is afternoon greetings, and "Oroin mend" is evening greetings.

...
In this vignette you'll learn how to set up your package for translation.
...
It also captures messages from the explicit translation functions `gettext()` and `ngettext().` (It does **not**, however, not translate `cat()`).
...
The advantage of the explicit style is that it's very clear which messges are ready for translated.
...
The following sections give some advice about how to write good messages, as inspired by the "[Preparing translatable strings](https://www.gnu.org/software/gettext/manual/html_node/Preparing-Strings.html#Preparing-Strings%20(Inspired%20by%20from%20))" section of the gettext manual.

### Write full sentences

Generally, you should strive to make sure that each message comes from a single string (i.e. lives within a single "").
Code like this is hard to translate for two reasons:
...
Secondly, and more importantly, prose is not like code: you can't reliably build up sentences from small fragments of text.
...
Unfortunately this breakdown doesn't work because the three greetings in French should be:

-   Good morning: Bonjour
-   Good afternoon: Bonjour
-   Good evening: Bonsoir

There are two problems: good morning and good afternoon both use bonjour (even though French has different words for morning and afternoon; bon aprés midi is used as a farewell), and the two word English phrases turn into single French words.
There's no way to generate the correct greetings by combining individual words:

A better approach would be to define a fixed number of time points so we can generate a full greeting for each:
...
This allows us the translator to generate three correct translations:
...
Greetings are particularly challenging to translate because of their great cultural variation; fortunately most of messages that you generate for R packages will not require this level of nuance.
...
-   `sprintf()` is built into base R, so is always available.
    The downside is that it can be hard to figure out the sentinels refer to and the syntax for rearranging components (which uses `1$`, `2$`) is somewhat arcane.

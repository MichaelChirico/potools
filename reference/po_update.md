...
`po_update()` updates existing `.po` file after the `.pot` file has
changed. There are four cases:

- New messages: added with blank `msgstr`.

- Deleted messages: marked as deprecated and moved to the bottom of the
  file.

- Major changes to existing messages: appear as an addition and a
  deletion.
...
- Minor changes to existing messages: will be flagged as fuzzy.
  ...
  using the old `msgid` as a potential reference, then delete the old
  translation and the `fuzzy` comment (c-format should remain, if
  present).
...
- lazy:
  ...
  If `TRUE`, only `.po` files that are older than their corresponding
  `.pot` file will be updated.

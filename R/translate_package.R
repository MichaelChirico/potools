...
#' templated messages (e.g., that use \code{\%s}), the user-provided message
...
#' \code{\%2$s} says "interpret the second input as a string". See
...
#' from \code{tools}, \code{\link[tools]{update_pkg_po}},
#' \code{\link[tools]{xgettext2pot}}, \code{\link[tools]{xgettext}}, and
#' \code{\link[tools]{xngettext}}). They should only be superficial (e.g.,
...
#' default (e.g. with \code{\link[tools]{update_pkg_po}}).
#'
...
#' (\code{\}), followed by a comma-separated list of argument names. All
...
#' \code{custom_translation_functions = list(R = c("gettext:...\domain",
...
#' the \code{\link{check_untranslated_src}} diagnostic, for example.
#'
#' \bold{Diagnostics:}
#'
...
#' \code{\link{get_message_data}}), evaluates whether these messages are
...
#' \code{\link{check_untranslated_cat}} has \code{diagnostic_tag =
#' "untranslated messaging calls passed through cat()"}.
#'
#' The output diagnostic result has the following schema:
#'
#' \enumerate{ \item \code{call}, \code{character}, the call identified as
#' problematic \item \code{file}, \code{character}, the file where \code{call}
#' was found \item \code{line_number}, \code{integer}, the line in \code{file}
#' where \code{call} was found \item \code{replacement}, \code{character},
#' \emph{optional}, a suggested fix to make the call "healthy" }
#'
#' See \code{\link{check_cracked_messages}},
#' \code{\link{check_untranslated_cat}}, and
#' \code{\link{check_untranslated_src}} for examples of diagnostics.
#'
#' \bold{Domains:}
#'
...
#' Romanian, etc. See \code{\link[base]{Sys.getlocale}} for some helpful tips
...
#' Details.
#' @param diagnostics A \code{list} of diagnostic functions to be run on the
...
#' @param copyright Character; passed on to \code{\link{write_po_file}}.
#' @param bugs Character; passed on to \code{\link{write_po_file}}.
...
#' @seealso \code{\link{get_message_data}}, \code{\link{write_po_file}},
#' \code{\link[tools]{xgettext}}, \code{\link[tools]{update_pkg_po}},
#' \code{\link[tools]{checkPoFile}}, \code{\link[base]{gettext}}
#' @references
